...
namespace Hotelaria {
    ...
    void ControladoraPersistenciaSolicitacaoHospedagem::salvar(const SolicitacaoHospedagem &status) {
        ...
        sqlite3_bind_text(stmt, 1, status.getChegada().toString().c_str(), -1, SQLITE_STATIC);
        sqlite3_bind_text(stmt, 2, status.getPartida().toString().c_str(), -1, SQLITE_STATIC);
        sqlite3_bind_int(stmt, 3, EnumConversor::StatusSolicitacaoHospedagemParaInteiro(status.getStatus()));
        sqlite3_bind_text(stmt, 4, status.getMotivoRecusa().c_str(), -1, SQLITE_STATIC);
        sqlite3_bind_int(stmt, 5, status.getHospedeId());
        sqlite3_bind_int(stmt, 6, status.getHotelId());
        sqlite3_bind_int(stmt, 7, status.getQuartoId());
    ...
    vector<SolicitacaoHospedagem> ControladoraPersistenciaSolicitacaoHospedagem::buscarPorEmail(const string &id) {
        ...
        const char *sql = R"(
        SELECT id, chegada, partida, status, motivo_recusa, id_hospede, hotel_id, quarto_id,
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            string partidaStr = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 3));
            StatusSolicitacaoHospedagem status = EnumConversor::InteiroParaStatusSolicitacaoHospedagem(
                sqlite3_column_int(stmt, 5));
            string motivo = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 3));
            string id_hospede = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 4));
            string hotel_id = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 5));
            string quarto_id = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 6));
    ...
    vector<SolicitacaoHospedagem> ControladoraPersistenciaSolicitacaoHospedagem::buscarPorStatus(int statusFiltro) {
        ...
        const char *sql = R"(
        SELECT id, chegada, partida, status, motivo_recusa, id_hospede, id_hotel, id_quarto,
        FROM solicitacoes_hospedagem
        WHERE status = ?;
    )";
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            string id_hospede = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 5));
            string id_hotel = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 6));
            string id_quarto = reinterpret_cast<const char *>(sqlite3_column_text(stmt, 7));
